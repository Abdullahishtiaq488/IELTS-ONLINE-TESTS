...
export function TipsCardGrid({ tipsCards }: TipsCardGridProps) {
    // Map the section titles to their corresponding blog slugs
    const slugMap: Record<string, string> = {
        "Reading Tips": "ielts-reading-guide",
        "Writing Tips": "ielts-writing-tips",
        "Listening Tips": "ielts-listening-strategies",
        "Speaking Tips": "ielts-speaking-success"
    };

    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            {tipsCards.map((card, index) => {
                // Get the corresponding blog slug based on the title
                const blogSlug = slugMap[card.title] || card.link.replace(/^\//, '');
